...
> TLDR; You can have roxygen automatically build your R data documentation from an external file.
...
We can make the storage and entry of our data documentation cleaner while enabling reuse by storing the data dictionary in a file separate from the R source code. Then we could pass our data dictionary through a function to convert it into a LaTeX-style `\tabular` object that can be manually pasted into a roxygen block (See http://r-pkgs.had.co.nz/man.html):
...
Now when we build the documentation with roxygen::document() `@eval` will evaluate the function and include a nicely formatted data dictionary table in our docs :tada:
